...
coolstore-inventory-service = Quarkus source, app-local GitOps state, pipeline assets, rollout notes, promotion notes, and rollback evidence
```

The biggest remaining gap is implementation readiness. The README set is strong as a design and planning artifact, but the app repository is not yet renamed, the Quarkus service does not exist, and no live cluster validation has been performed for the planned developer workflow stages.
...
- the current `coding-exercises` shell and Python starter files still pass syntax checks.
...
| Developer workflow index | Strong overview, source posture, stage table, supporting analysis links, implementation policy, and accepted next decisions | High | Needs updates whenever frontend, pipeline-template, eval, standards-corpus, or live-validation choices are made. |
...
| Stage 140 | Strong target-service framing around `coolstore-inventory-service`, Red Hat build of Quarkus 3.27.x, Java 21, and the first PostgreSQL path | Medium-high | Needs source scaffold, Java tests, and catalog metadata. |
| Stage 150 | Strong single-repo app-local GitOps and pipeline boundary | Medium | Needs pipeline template source, image update strategy, and validation commands. |
...
| Quarkus target-service assessment | Strong target-service decision record | High | Needs concrete directory conventions and the selected pipeline template. |
| `coding-exercises` app-repo plan | Good first planning artifact for the future renamed service repo | Medium-high | App repo still contains Python exercise content and no Quarkus scaffold. |
...
- Stage 150 adopts app-local `gitops/` and `tekton/` directories, but the exact pipeline template source is still open.
...
- single service repository with source, app-local GitOps, pipeline assets, and evidence;
...
- Quarkus app scaffold in the future renamed service repository;
- pipeline and supply-chain resources;
- Developer Hub catalog entities for the new app and GitOps resources;
...
2. Add app-repo planning for `AGENTS.md`, `catalog-info.yaml`, service README, `gitops/`, `tekton/`, and evidence documentation.
3. Define the Stage 150 pipeline template source and image update strategy.
4. Define the Stage 155 evidence bundle for the first service image.
