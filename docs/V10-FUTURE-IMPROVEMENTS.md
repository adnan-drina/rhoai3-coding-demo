...
| O-SPECIMEN-CRIT | 📋 | **Wave 2 GO 2026-07-31:** provisioned `petclinic-rest-v1` via RHDH Application migration template (scaffolder 5/5 green); destination https://github.com/adnan-drina/petclinic-rest-v1; Dev Spaces workspace ready (`modernized=/projects/modernized`, `legacy=/projects/legacy`); ns `petclinic-rest-v1-dev`. **Stamp ✅ 2026-07-31:** destination `migration.yaml` commit `781e3aa` — `legacyPackage=org.springframework.samples.petclinic`, `targetPackage=com.demo`, `acceptance.path=/petclinic/api/vets`, `collection=_array`, `getter=getAllVets`, `service=ClinicService`, `itemType=VetDto`, `idFields=[id]`, `needsDatabase=true` (dbService/dbName omit → PROJECT_KEY defaults), `analysis.mode=source-only`, `preserve=[petclinic.security.enable]`, `forbidden=[]`, conservative `targetContract` (behavior-changing cart flags false). Cart workspace `7b54999` remains idle — do not remount. Primary freeze-fork: https://github.com/adnan-drina/spring-petclinic-rest-legacy (`main` = upstream **v2.6.2** / SHA `517a399`). **PROBE ✅** source-only ≡ full — **29 rules / 151 incidents**. **DECISION-DB ✅ Postgres**. Harness pre-work banked: O-ACCEPTGEN/O-PKGPREFIX, O-MAPPINGS-PETCLINIC, freeze-fork, PROBE, DECISION-DB, O-ACCEPTARRAY, O-DEVDBURL. **Harness / outer-loop not started.** Status stays 📋 until first live exercise. |
