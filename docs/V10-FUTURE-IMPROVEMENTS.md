...
| O-SPECIMEN-CRIT | 📋 | **Wave 2 GO 2026-07-31:** provisioned `petclinic-rest-v1` via RHDH Application migration template (scaffolder 5/5 green); destination https://github.com/adnan-drina/petclinic-rest-v1; Dev Spaces workspace starting (`modernized=/projects/modernized`, `legacy=/projects/legacy`); ns expected `petclinic-rest-v1-dev`. **Next before harness:** stamp petclinic acceptance contract into destination `migration.yaml` (`_array`, path, packages, preserve security toggle, `analysis.mode`); confirm golden harness has O-ACCEPTARRAY/O-DEVDBURL; **do not start outer-loop until stamp done**. Cart workspace `7b54999` remains idle — do not remount. Live scaffold still cart-default (expected): `legacyRepoUrl=https://github.com/adnan-drina/spring-petclinic-rest-legacy`, `needsDatabase=true`, `POSTGRESQL_DATABASE=petclinic-rest-v1`, JDBC `jdbc:postgresql://petclinic-rest-v1-postgres:5432/petclinic-rest-v1`, but packages/path/preserve still Coolstore. Primary freeze-fork: https://github.com/adnan-drina/spring-petclinic-rest-legacy (`main` = upstream **v2.6.2** / SHA `517a399`). **PROBE ✅** source-only ≡ full — **29 rules / 151 incidents**; `analysis.mode=source-only`. **DECISION-DB ✅ Postgres**. Specimen contract to stamp: `path=/petclinic/api/vets`, `collection=_array`, `getter=getAllVets`, `service=ClinicService`, `itemType=VetDto`, `needsDatabase=true`; preserve context-path + `petclinic.security.enable=false`. Pre-work banked: O-ACCEPTGEN/O-PKGPREFIX, O-MAPPINGS-PETCLINIC, freeze-fork, PROBE, DECISION-DB, O-ACCEPTARRAY, O-DEVDBURL. Status stays 📋 until stamp + first live exercise. |
