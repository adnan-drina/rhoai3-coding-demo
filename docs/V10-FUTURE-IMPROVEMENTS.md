...
| O-SPECIMEN-CRIT | 📋 | **Ready for peer review → then provision** (decision HOLDs cleared). Primary: `spring-petclinic-rest` @ freeze-fork https://github.com/adnan-drina/spring-petclinic-rest-legacy (`main` = upstream **v2.6.2** / SHA `517a399`, Boot 2.6.2; not upstream `main`/Boot 4). **DECISION-UI N/A** (REST-primary). **PROBE ✅** (2026-07-31): source-only ≡ full — **29 rules / 151 incidents** (mandatory 27 + potential 2; optional 0); **K3 mix YES**; `analysis.mode=source-only`; jar-full abandoned. **DECISION-DB ✅ Postgres** — `acceptance.needsDatabase: true` when provisioned; platform PostgreSQL (`DEV_DB_URL` / coolstore-postgres shape); port schema from HSQLDB/MySQL scripts (v2.6.2 has no postgres scripts); do not make MySQL/HSQLDB primary runtime. Pre-work shipped: O-ACCEPTGEN/O-PKGPREFIX (`c15ecf9`), O-MAPPINGS-PETCLINIC (`e695fc3`), freeze-fork note (`4e32d4a`), PROBE bank (`e3ebf58`). Handoff for peer review: `tmp/KAI-WAVE1-REVIEW.md` § *Specimen readiness handoff — spring-petclinic-rest-legacy (2026-07-31)*. **Do not** create workspace / start run until peer ACK. Status stays 📋 until provisioned. |
