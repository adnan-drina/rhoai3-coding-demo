...
| O-M3ROUTE | ✅ | Fixed 2026-08-02; **default flipped 2026-08-03 (W4-175):** `WORKER_M3_FIRST` default `true` — Qwen drafts M3 first; MiniMax backstop (`M3_ORCH_BACKSTOP=2`). Set `false` for MiniMax-first. Instrument O-M3ROUTE. |
...
| O-PREPARCHEXIT | ⬜ | W4-150a: wave-2 clean stop + `prep-fresh-rerun` left `migration/run-archives/` empty and wiped `/tmp` seat logs before/without a durable EXIT scoop — wave-1 archive also gone; m2-corpus has no wave-2 case. Confirm wipe/prep path does not bypass outer EXIT archive (or scoop `/tmp`→host before reset). Pair O-TMPARCHIVE. Host mitigation: `tmp/m2-wave2-quota-evidence-*`. P3. |
...
| O-M3PIPEFIELD | ✅ | Fixed 2026-08-03 (W4-168 P1): `plan-lint.py` `normalize_m3_pipe_fields` strips leading `\|/>` from Class/Shape/Port/Owns/… lines and rewrites tasks.md before checks — stops false `O-SHAPEDECL` loops when MiniMax emits markdown-table pipes. Live probe S02: 320→0 pipes, Shape-miss 0. Instruments `m3pipefield-wire-ok` / `m3pipefield-strip-ok`. Prefer normalize-over-relax (task-packet/m3-all-lint stay aligned). |
| O-ACCEPTSUBST | ⬜ | W4-168 P2: Acceptance that only names gate vocabulary (`plan-lint green; sensors green`) is ceremonial — lint should require a task-specific predicate. Bank before PAUSE-3. Pair O-FINDINGFORM. |
