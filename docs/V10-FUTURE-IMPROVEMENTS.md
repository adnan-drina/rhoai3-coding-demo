...
| O-ORCH429BACKOFF | ⬜ | Wipe M2 2026-08-02: outer logged "MiniMax rate limit … supervisor backs off 15m on orch 429s" then launched M2 a2 immediately. Honor real sleep/backoff before next orch seat, or stop claiming 15m. |
| O-M2-429 | ⬜ | Wipe M2 a1: hard 429×3 mid-seat left incomplete briefs → lint RED (PROFILE soft-429 recovered). Pair orch 429 handling with partial-artifact refuse / same-attempt resume after backoff (not burn attempt on truncated write). |
