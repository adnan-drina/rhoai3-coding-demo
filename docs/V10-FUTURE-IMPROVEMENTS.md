...
| O-SPECIMEN-CRIT | 📋 | **Harness-side Poll 81 B1 cleared; still DO NOT create workspace** until operator green-light after peers see the Implementing note. Primary: `spring-petclinic-rest` @ freeze-fork https://github.com/adnan-drina/spring-petclinic-rest-legacy (`main` = upstream **v2.6.2** / SHA `517a399`, Boot 2.6.2; not upstream `main`/Boot 4). **DECISION-UI N/A** (REST-primary). **PROBE ✅** (2026-07-31): source-only ≡ full — **29 rules / 151 incidents** (mandatory 27 + potential 2; optional 0); **K3 mix YES**; `analysis.mode=source-only` (E2 wired in scaffold `migration.yaml` + `analyze.sh`); jar-full abandoned. **DECISION-DB ✅ Postgres** — `acceptance.needsDatabase: true` when provisioned; platform PostgreSQL; E3: M1 documents postgres port; **early M4 owns schema+seed** (acceptance 404 if empty). **Poll 81 B1 ✅** bare-array `_array` sentinel (see O-ACCEPTARRAY). Peer conditional ACK satisfied for harness side. Specimen contract intent (provision, not live yet): `path=/petclinic/api/vets`, `collection=_array`, `getter=getAllVets`, `service=ClinicService`, `itemType=VetDto`, `needsDatabase=true`; preserve context-path + `petclinic.security.enable=false`. Pre-work: O-ACCEPTGEN/O-PKGPREFIX (`c15ecf9`), O-MAPPINGS-PETCLINIC (`e695fc3`), freeze-fork (`4e32d4a`), PROBE (`e3ebf58`), DECISION-DB handoff (`ffd1bf7`). Status stays 📋 until provisioned. |
