...
| O-SPECIMEN-CRIT | 📋 | **Poll 80 pick:** `redhat-developer-demos/spring-petclinic` (Spring Boot 2.3 / javax, hits K3+K1-SHARED+K8+K6). Contingent on PROBE (M1-style kantra mix) + operator DECISION-UI (Qute vs Thymeleaf ext vs waive) + DECISION-DB (postgres scripts vs MySQL `DEV_DB_URL`). Sequence: worker revival + UXLOG Wave A ✅ first; then O-ACCEPTGEN/O-PKGPREFIX; then provision. |
