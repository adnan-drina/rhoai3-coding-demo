...
| O-SPECIMEN-CRIT | 📋 | **Poll 80b/80c AGREE:** primary specimen `spring-petclinic/spring-petclinic-rest` **tag `v2.6.2`** (last javax-era; not upstream `main`/Boot 4). Operator freeze-fork (e.g. `adnan-drina/spring-petclinic-rest-legacy` with v2.6.2 as main) — coolstore-cart-legacy pattern. **DECISION-UI N/A** (REST-primary; no Thymeleaf). MVC fork (`redhat-developer-demos/spring-petclinic`) remains fallback if UI spectacle wanted. HOLD provision until: (1) operator freeze-fork, (2) PROBE source-only vs binary/full on that fork, (3) **DECISION-DB** (HSQLDB default vs MySQL/postgres scripts). O-ACCEPTGEN/O-PKGPREFIX ✅ (`c15ecf9`). |
| O-MAPPINGS-PETCLINIC | ⬜ | Poll 80b: bank Vila meetup mappings into MAPPINGS.md + architecture-profile rubric — AOP→REDESIGN, derived queries→`@Query`/PanacheRepositoryBase, injection visibility, metrics preserve-tokens, security SpEL→`@RolesAllowed` + Elytron JDBC, MockMvc→RestAssured rewrite, accepted deviations (CORS/OpenAPI/JMX). Also document standards path vs Spring-compat extensions in migration-harness skill. |
