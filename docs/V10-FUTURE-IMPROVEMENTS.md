...
| O-SPECIMEN-CRIT | 📋 | **Poll 80b/80c AGREE:** primary specimen `spring-petclinic/spring-petclinic-rest` **tag `v2.6.2`** (last javax-era; not upstream `main`/Boot 4). **Freeze-fork LIVE:** https://github.com/adnan-drina/spring-petclinic-rest-legacy (`main` = upstream v2.6.2, Boot 2.6.2) — coolstore-cart-legacy pattern. **DECISION-UI N/A** (REST-primary; no Thymeleaf). MVC fork (`redhat-developer-demos/spring-petclinic`) remains fallback if UI spectacle wanted. Remaining HOLD: (1) PROBE source-only vs binary/full on that fork, (2) **DECISION-DB** (HSQLDB default vs MySQL/postgres scripts). O-ACCEPTGEN/O-PKGPREFIX ✅ (`c15ecf9`). |
