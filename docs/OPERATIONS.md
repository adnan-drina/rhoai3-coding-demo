...
Grafana queries OpenShift monitoring through the Thanos Querier using a `grafana-sa` service account with the `cluster-monitoring-view` role. The `GrafanaDatasource` manifest keeps only a placeholder token in Git. A Stage 040 sync job waits for the Grafana deployment rollout, mints the runtime token, patches `GrafanaDatasource/prometheus`, and nudges the dashboard/datasource CRs to reconcile after Grafana restarts. Argo CD ignores only that generated token field. Grafana uses a small persistent volume so the dashboard and datasource survive sidecar restarts. If dashboards show `401 Unauthorized`, re-run Stage 040 sync and validate that the datasource can query OpenShift monitoring.
