...
| The loop refuses `HARNESS_RELEASE_MISMATCH` or `MODEL_PROFILE_MISMATCH` (exit 2 from `run-verify.sh`, `advance.py` or `k4_mint.py`) | The run executes the harness release and the model profile it was created with (`/projects/.platform/run-control/release.json`, `profile.json`). A mid-run change is either a new run or an assisted continuation: install it, then record it with `python3 -c 'import sys; sys.path.insert(0,".hermes/lib"); from planner.run_control import rebase_release; rebase_release(".", "<why>")'`. The journal (`run-control/journal.jsonl`) keeps the event permanently. |
...
| Admission names `RUN_ACTIVATION_MISSING` or `RUN_ACTIVATION_FOREIGN` | The run's activation is run control (`/projects/.platform/run-control/activation.json`, read-only, journaled), never `.hermes/pins.json`. MISSING names the last journal event (run, bind time, bundle). Restoring the file from a backup is an assisted continuation. FOREIGN is a seal from another run: create the run again. |
