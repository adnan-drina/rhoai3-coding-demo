...
**Root cause (CORRECTED — my first write blamed fidelity; the run's own logs disprove that).** Over the entire run, harvest-fidelity flagged ONLY `ShoppingCart.java` (a model), 8 times, all the #5 whitespace false positive. It NEVER flagged `ShoppingCartServiceImpl`, `CartEndpoint`, or `PromoService` — the discriminator (`harvest-fidelity.py` L62) skips any class carrying `@ApplicationScoped/@Inject/@Path/@RegisterRestClient`, and all three do. **So fidelity does NOT block hardening the services/endpoints — it structurally exempts them.** The service could have shipped thread-safe, idempotent, and validated without ever tripping a fidelity check. Fidelity was a red herring; the real cause is upstream:
1. **The M3 plan tasks specified "convert faithfully"** — they set the LEGACY behavior as the target, not a modern one.
2. **The characterization tests were written to pin legacy assertion values** ("legacy assertion values preserved") — so they actively ENFORCE the defects; hardening the code turns them red.
3. The MAPPINGS "Production-grade defaults" were soft guidance losing to a plan and a test suite that both demanded legacy behavior.

**Why the gates are all green:** legacy `HashMap`/create-on-GET/dedupe-order are not sonar rules; the services are fidelity-exempt (so their structure is unconstrained); and the characterization tests pin legacy behavior as correct. Every signal says "clean" because the plan and tests DEFINED clean as "same as legacy."

**Fix at the SOURCE (supersedes the earlier "append a hardening story" — that was a plaster solving a non-existent fidelity conflict).** Decide the modernization TARGET up front and split work by class role:
- **Behavior-preserving quality** (thread-safety, cache guard, resource/error-path handling): same observable behavior → characterization tests still pass, services already fidelity-exempt → NO conflict. Make these a HARD plan requirement / wiring-sensor check (not skill prose). Closes #1, #2, #5, and the error-mapping half of #4, code written once.
- **Behavior-changing contract** (GET→404, invalid quantity→400): genuinely changes the API → an explicit TARGET-CONTRACT decision at M1/M2; when adopted, M3 writes the characterization tests to the TARGET, not the legacy. Closes #3, #4 with no rewrite, because the tests never pinned the old behavior.
- **Class-role split (the organizing principle M1 can emit):** HARVEST classes (models, DTOs, utilities — faithful, fidelity-on, tests pin legacy) vs REDESIGN classes (services, endpoints, config — target decided up front, tests pin target, production-grade by construction, fidelity already exempts). The current process treats everything as harvest-with-legacy-behavior; that single default is the bug.
