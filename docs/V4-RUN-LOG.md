...
6. *The sfix path accepts a commit without re-verifying the sensor* — **the highest-value finding, found live at T-017**: the fidelity RED (the whitespace false positive above) was handed to a sensor-fix session, which MISDIAGNOSED it — it never touched `ShoppingCart` (the flagged file), instead added unrelated exception-handling to `ShoppingCartServiceImpl` + a test, and committed as `T-017 sensor fix: Add exception handling for CatalogService calls in tests`. The supervisor's post-commit logic checks only `committed "<prefix> sensor fix"` (the commit exists) and returns success — it does NOT re-run the sensor. Result: run proceeded to T-018 with fidelity still RED, AND unplanned scope (exception handling) was injected into a class a later task (T-020) will rewrite. Fix (next run): after the sfix commit, re-run the sensor; if still red, burn a retry or record honest debt — never treat "a commit with the right prefix" as proof the red cleared. This gap is independent of the false positive: even a real red could be papered over by a misdirected fix. Compounds with #5 — the false positive re-fires at every milestone, each spawning a misdiagnosing sfix that adds more unplanned change.
