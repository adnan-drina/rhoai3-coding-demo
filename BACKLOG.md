...
- [ ] **Stage 080 implementation phase — Trusted Software Factory pattern** — base setup exists (stage dirs, Pipelines + RHTAS operators via GitOps). Remaining: anchor: [Trusted software factory: Building trust in the agentic AI era](https://developers.redhat.com/articles/2026/05/13/trusted-software-factory-building-trust-agentic-ai-era). Story: after Stage 070's autonomous agents produce code, the factory proves what was built and how — SLSA provenance and sigstore signatures/attestation for AI-generated changes, SBOMs, Trusted Profile Analyzer for vulnerability triage, Red Hat Trusted Software Supply Chain (Konflux-based) pipelines, hardened images and trusted libraries. The article is strategy-level with no reference implementation, so before creating the stage: identify deployable components for a demo sandbox (RHTSSC availability, TAS, TPA operators), a concrete pipeline for the Stage 070 migrated app, and a validation path. Create the stage directory only with that plan per the stage rules.
