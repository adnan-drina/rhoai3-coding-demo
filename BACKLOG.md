...
- [ ] **llm-d router-scheduler modelcar exhausts co-located 100G workers** — This operator build stamps the 30-37 GiB modelcar (tokenizer + modelcar sidecars) onto the CPU **router-scheduler** pod regardless of the EndpointPicker plugin config (RFE candidate). The scheduler pods carry **no `ephemeral-storage` request**, so the kube-scheduler is disk-blind and can place *both* models' scheduler pods on one worker; the sandbox CPU workers are only 100G, so the second modelcar pull fails with `no space left on device` (ImagePullBackOff). Observed on cluster-kjbwr: nemotron + qwen schedulers both landed on one worker; the other three had 81G free. Deleting the pod re-schedules it to the same full node (disk-blind). **FIXED (2026-07-21):** added `podAntiAffinity` (preferred, weight 100) on `spec.router.scheduler.template` of both the nemotron and qwen LLMInferenceServices, repelling other `llminferenceservice-router-scheduler` pods from the same node (`topologyKey: kubernetes.io/hostname`) - so the modelcars spread across workers and never co-locate. This is cleaner than extending worker disk (no infra change). Deeper fix still open: bring-your-own EPP via `pool.endpointPickerRef` to drop the modelcar from the scheduler pod entirely (tracked separately).
