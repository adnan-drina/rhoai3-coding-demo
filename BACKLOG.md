...
- [ ] **Scaffold dual source of truth** — `golden-repos/agentic-quarkus-scaffold/` (bootstrap force-push source) and the live `agentic-quarkus-scaffold` repo drift apart; today they diverged by a full generation. Either make bootstrap clone the repo instead of the checked-in copy, or add a CI check that diffs them.
...
- [ ] **Stage 080 elevated key is ai-developer-scoped** — `migiq/agentic/elevated-key-provisioning.yaml` does not cover `wksp-ai-admin`; the second demo user cannot run the migration flow.
