...
- [ ] **Standalone platform RHBK (realm `platform`)** — replace the transitional RHDH↔MTA-Keycloak brokering with a platform-owned RHBK in stage 050 `identity/`; unblocks the `overlays/slim` variant (platform without MTA). Needs live validation of realm/client/OAuth wiring before it lands in GitOps.
