...
- [ ] **Stage 050 implementation recipe** — adopt showroom modules 5-6 as the reference: Tekton pipeline for the Stage 080 migrated app with SonarQube gate, Tekton Chains SLSA attestation, TAS signing, SBOM generation, TPA SBOM/vulnerability management. Implementation plan: docs/PLAN-implementation-wave-2.md (workstream B).
- [ ] **Stage 050 self-service template** — RHDH software template scaffolding a component + governed workspace (showroom module 4 pattern) and topology wrap-up view (module 6). Implementation plan: docs/PLAN-implementation-wave-2.md (workstream C).
