...
| Migration Toolkit for Applications | `mta-*` | `mta-release-notes`, `mta-install`, `mta-lightspeed`, `mta-cli`, `mta-ui`, `mta-vscode`, `mta-intellij`, `mta-rules` | Official-doc-backed MTA 8.2 application modernization: release notes, installation, Developer Lightspeed AI, CLI/UI/VS Code/IntelliJ analysis tools, and custom rule development |
| Hermes Agent | `hermes-*` | `hermes-about`, `hermes-configuration`, `hermes-managed-scope`, `hermes-skills`, `hermes-hooks`, `hermes-kanban`, `hermes-sessions`, `hermes-cli`, `hermes-tools`, `hermes-delegation`, `hermes-goals`, `hermes-personality`, `hermes-self-improvement`, `hermes-one-three-one-rule` | Official-doc-backed Hermes Agent (Nous Research) guidance for stage 080 implementation; consult the matching skill BEFORE designing or reviewing any Hermes-touching change — official docs are product authority (all eight captured and validated 2026-08-12; each skill's `references/source-capture.md` carries provenance, capture date, and open items) |
...
| Agent Skills Standard | `agentskills-*` | `agentskills-authoring`, `agentskills-descriptions`, `agentskills-evaluation` | Official agentskills.io guidance for creating and modifying Agent Skills. **Consult BEFORE writing or editing any SKILL.md in this repository** — `.agents/skills/` is the standard's default discovery path, so its format rules (name ≤64 chars, description ≤1024 chars, SKILL.md <500 lines) apply to our own skills |
...
| 080 migration | `rhoai-maas-governance`, `ocp-authentication-identity-providers` (Keycloak), MTA product docs, the full `hermes-*` skill family (see Shared Skills) |
