...
Stage 070 answers with two complementary paths on the same platform:
...
- The Coolstore sample for the assisted path is the external `konveyor-ecosystem/coolstore` repository — it is not imported by Stage 070 GitOps. The MTA VS Code extensions used in Dev Spaces are provisioned by Stage 050 workspace assets (`mca-coolstore` DevWorkspace), not by this stage.
- MaaS usage dashboards showing token consumption depend on Stage 040 observability — they are not configured by Stage 070.
...
- **[Red Hat OpenShift Dev Spaces](https://www.redhat.com/en/technologies/cloud-computing/openshift/dev-spaces)** hosts the agentic migration workspace. The MTA VS Code extensions are provisioned in Stage 050 workspace assets.
...
Flow dependency: Stage 050 (AI-Assisted Development).
