...
| Parked requeue | `.hermes/_park/requeue/` — **not a skill**; **authoring-only**. Dest clones via `scripts/bootstrap-migration-scaffold-v2.sh` omit `_park`. Chaos never dest. `_park/mint/` deleted after K4. Do not mkdir empty `kernel/` from here. |
