...
| Paved-road | `.hermes/skills/paved-road/` — kind index (`paved-road-m1` … `paved-road-m4`; `paved-road-m5`: eligibility-checked start → prepare candidate → observe app-push → assert Deployment/Route → live Route acceptance → compose M5 verdict). `steps.json` generates `audit.json`. M5 is an assisted continuation after M4 close with a separate budget; the M4 worker never dest-dispatches it. Application values live in `delivery.yaml`. |
...
| `paved-road-m5` | paved-road | M5 delivery index; pin on M5-A/B/C; eligibility-checked start after M4 close |
