...
trap 'rm -f "${BODY_MD}"' EXIT
{
  ...
  echo "**In-loop testCompile invariant (S-010 Class A #1b / Deputy E-115113Z):** \`python3 .hermes/skills/auditability-repeatability/scripts/stamp-implementer-checkpoint.py --completed src/test/**\` **REFUSE**s unless \`python3 .hermes/skills/auditability-repeatability/scripts/run-test-compile-gate.py\` (\`mvn -q test-compile\`) is green for that advance — not advisory prose. Red compile ⇒ fix before stamp/next. Scaffold ships assertj-core + rest-assured (\`migration/contracts/test-toolchain.md\`)."
