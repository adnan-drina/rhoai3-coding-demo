...
trap 'rm -f "${BODY_MD}"' EXIT
{
  ...
  echo "**AD-011 / R-AD011.2 overlay (Architect E-20260810T185500Z):** if \`extensions/<skill>/references/*\` exists (e.g. \`extensions/spring-to-quarkus-patterns/references/jdbc-anti-essay.md\`), \`skill_view\` the **base** skill **and** the extension path — Hermes has no merge. Never SOUL. See \`migration/contracts/ad011-skill-extension.md\` + \`workshop-extensions/README.md\`."
  ...
  echo "**Hard invoke (AD-002G P0.3):** run \`/spring-to-quarkus-patterns\` (or equivalent \`skill_view\` on that skill) before first destination edit; then open needed \`references/*\` (rest / di-config / persistence / testing / security-config). For S-010/test work open \`references/testing.md\` §Failure/Import/Mock procedures + golden fixture \`migration/fixtures/testing/golden-rest-controller/PetTypeRestControllerTests.java\`."
