...
**Orchestration:** dest-init mints M1 with `--skill paved-road-m1`. This
leaf loads via `skill_view` from that index `steps.json`. Do **not** pin
this leaf on the card. Kanban owns the task, role, budget, and recovery.
