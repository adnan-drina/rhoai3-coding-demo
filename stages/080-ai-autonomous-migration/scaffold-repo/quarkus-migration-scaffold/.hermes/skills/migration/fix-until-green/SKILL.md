...
| a **unit** card on the `package` gate whose issued obligation is no longer reported, whose sealed members all assess clean, and whose gate now stops ONLY on located failures in files the unit does not reach (the reach test `amend-scope.py` refuses the card on) | accepted (`gate_handoff`: the obligation it handed off to, where it is, and that the closing card still owes package + boot on one artifact) | the build reports one failure at a time, so the unit cannot make the gate pass and no card can while the unit holds the slot (v12 `t_b33f25fa`: the fragment implementations were right and the build then stopped on an unrelated SpEL `@Value`); the new obligation is the next card. An unlocated or set-wide failure, one in a file the unit reaches, or any member not assessed clean keeps the card `VERIFICATION_PENDING` (`unproven-repair`) |
