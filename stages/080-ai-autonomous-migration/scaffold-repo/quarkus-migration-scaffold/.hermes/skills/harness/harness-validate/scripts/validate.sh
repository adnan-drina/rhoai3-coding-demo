...
case "${1:-}" in
  -h|--help)
    cat <<'USAGE'
validate.sh — specimen-free harness validation suite (skill harness-validate, AD-H S7).
...
Sections covered (in order):
  ...
  auditability-repeatability (AD-H S19)   R-M3.5-8 POM / dependency_wait
...
Examples:
  ...
  bash .hermes/skills/harness/harness-validate/scripts/validate.sh --help

Exit codes:
  0  pass — every section passed ("OK: harness-validate passed")
  1  BLOCK — one or more sections failed ("harness-validate FAILED" on stderr)
...
bash "${SKILLS}/harness/harness-validate/scripts/check-no-hermes-context-override.sh" || rc=1

echo "== sdd-readiness (+ §S.6) =="
bash "${SKILLS}/sdd/sdd-readiness/scripts/check-readiness.sh" || rc=1
sdd_tmp="$(mktemp -d)"
...
bash "${SKILLS}/gates/domain-gates/scripts/run-admission.sh" "${ROOT}" || rc=1
...
python3 "${SKILLS}/gates/domain-gates/scripts/parse-pit-mutations.py" \
  "${ROOT}/migration/fixtures/pit-dry-run/mutations.xml" || rc=1
if python3 "${SKILLS}/gates/domain-gates/scripts/parse-pit-mutations.py" \
...
g1op_tmp="$(mktemp -d)"
g1op="${SKILLS}/gates/domain-gates/scripts/check-g1-acceptance-operand.py"
...
python3 "${SKILLS}/analysis/mta-analysis/scripts/validate-findings-schema.py" \
...
python3 "${SKILLS}/harness/grounded-generation/scripts/check-citation.py" "${ROOT}" || rc=1
gg_tmp="$(mktemp -d)"
...
if python3 "${SKILLS}/harness/grounded-generation/scripts/check-citation.py" "${gg_tmp}" >/dev/null 2>&1; then
...
if python3 "${SKILLS}/gates/validation-release-gates/scripts/check-verdict-routing.py" "${vr_tmp}" >/dev/null 2>&1; then
...
printf '%s\n' '{"phase":"M4","verdict":"PROVISIONAL_ACCEPT","g1_kill_ratio":"pending_threshold","ship":false}' \
  > "${vr_tmp}/migration/verdicts/bad.json"
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-verdict-routing.py" "${vr_tmp}" || rc=1
...
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-verdict-routing.py" "${vr_tmp}" || rc=1
# factory M5 oracle
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-factory-m5.py" "${ROOT}" || rc=1
...
if python3 "${SKILLS}/gates/validation-release-gates/scripts/check-candidate-promote.py" "${vr_tmp}" >/dev/null 2>&1; then
...
printf '%s\n' '{"phase":"factory","status":"factory_ready","candidate_sha":"abcdef1","factory_result":"pass","promoted_to_main":false}' \
  > "${vr_tmp}/migration/preflight/factory.json"
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-candidate-promote.py" "${vr_tmp}" || rc=1
# side-effect recovery idle + persisted-data idle
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-side-effect-recovery.py" "${ROOT}" || rc=1
python3 "${SKILLS}/gates/domain-gates/scripts/check-persisted-data-contract.py" "${ROOT}" || rc=1
...
if python3 "${SKILLS}/gates/validation-release-gates/scripts/restore-or-refuse-requeue.py" "${f4_tmp}" \
...
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-workspace-clean.py" "${f4_tmp}" || rc=1
unset HERMES_HOME
rm -rf "${f4_tmp}" "${f4_home}"

# SCOPED_ACCEPT gate (finding 3)
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-accept-scope.py" "${ROOT}" || rc=1
printf '%s\n' '{"phase":"M5","verdict":"ACCEPT","accept_kind":"full","entry_point_descope_count":2}' \
  > "${vr_tmp}/migration/verdicts/m5.json"
if python3 "${SKILLS}/gates/validation-release-gates/scripts/check-accept-scope.py" "${vr_tmp}" >/dev/null 2>&1; then
...
printf '%s\n' '{"phase":"M5","verdict":"SCOPED_ACCEPT","accept_kind":"scoped","entry_point_descope_count":2}' \
  > "${vr_tmp}/migration/verdicts/m5.json"
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-accept-scope.py" "${vr_tmp}" || rc=1
rm -rf "${vr_tmp}"

echo "== external_dirs (AD-S relocate) =="
python3 "${SKILLS}/sdd/specify-workspace-init/scripts/check-external-dirs.py" "${ROOT}" || rc=1
ed_tmp="$(mktemp -d)"
...
if HERMES_HOME="${ed_tmp}/relocated" HERMES_CONFIG="${ed_tmp}/cfg/config.yaml" \
  python3 "${SKILLS}/sdd/specify-workspace-init/scripts/check-external-dirs.py" "${ed_tmp}" >/dev/null 2>&1; then
...
HERMES_HOME="${ed_tmp}/relocated" HERMES_CONFIG="${ed_tmp}/cfg/config.yaml" \
  python3 "${SKILLS}/sdd/specify-workspace-init/scripts/check-external-dirs.py" "${ed_tmp}" || rc=1
rm -rf "${ed_tmp}"
...
python3 "${SKILLS}/sdd/sdd-readiness/scripts/check-operand-count.py" "${ROOT}" \
  "${ROOT}/migration/fixtures/story-sizing/ar-size-good.json" || rc=1
if python3 "${SKILLS}/sdd/sdd-readiness/scripts/check-operand-count.py" "${ROOT}" \
...
python3 "${SKILLS}/gates/validation-release-gates/scripts/check-wall-exit-eval.py" "${wall_tmp}" \
  --task-id t_fixture_wall --trigger timed_out --require-test-compile || rc=1
if python3 "${SKILLS}/gates/validation-release-gates/scripts/check-wall-exit-eval.py" "${wall_tmp}" \
...
if python3 "${SKILLS}/harness/auditability-repeatability/scripts/check-test-write-checkpoint-lag.py" \
...
if python3 "${SKILLS}/harness/auditability-repeatability/scripts/stamp-implementer-checkpoint.py" \
  "${tc_tmp}/migration/runs/t_tcgate/checkpoint.json" \
...
python3 "${SKILLS}/harness/auditability-repeatability/scripts/stamp-implementer-checkpoint.py" \
  "${tc_tmp}/migration/runs/t_tcgate/checkpoint.json" \
  --completed src/test/java/com/demo/ATests.java --skip-test-compile-gate || rc=1
python3 "${SKILLS}/harness/auditability-repeatability/scripts/check-implementer-checkpoint.py" \
...
python3 "${SKILLS}/harness/auditability-repeatability/scripts/check-body-digest-match.py" "${ROOT}" \
  --body "${ROOT}/migration/fixtures/body-digest/ar-digest-good/body.json" \
  --expect "${DIGEST}" || rc=1
if python3 "${SKILLS}/harness/auditability-repeatability/scripts/check-body-digest-match.py" "${ROOT}" \
  --body "${ROOT}/migration/fixtures/body-digest/ar-digest-good/body.json" \
...
python3 "${SKILLS}/sdd/sdd-readiness/scripts/check-persistence-bom.py" "${ROOT}" || rc=1
python3 "${SKILLS}/sdd/sdd-readiness/scripts/check-compile-deps-preflight.py" "${ROOT}" || rc=1
dep_tmp="$(mktemp -d)"
mkdir -p "${dep_tmp}/migration/verdicts"
python3 "${SKILLS}/gates/validation-release-gates/scripts/apply-dependency-wait-hold.py" \
...
python3 "${SKILLS}/harness/phase-dispatch/scripts/assert-bundle-skills-exist.py" \
  "${ROOT}" --bundle m3-implementer || rc=1

echo "== BANK-DEST-INV-HARDINVOKE-1 (RW-2) =="
python3 "${SKILLS}/sdd/sdd-readiness/scripts/assert-dest-inventory-hardinvoke.py" \
  "${ROOT}" || rc=1
...
python3 "${SKILLS}/sdd/sdd-readiness/scripts/check-jdbc-deps-preflight.py" "${ROOT}" || rc=1
...
cp = subprocess.run(
    [sys.executable, str(root / ".hermes/skills/sdd/sdd-readiness/scripts/check-operand-count.py"),
