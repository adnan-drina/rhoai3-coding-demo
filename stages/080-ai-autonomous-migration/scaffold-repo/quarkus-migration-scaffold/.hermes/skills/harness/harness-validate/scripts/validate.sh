...
bash "${SKILLS}/harness-validate/scripts/check-no-hermes-context-override.sh" || rc=1
...
echo "== AD-012 / R-SK.5+R-SK.9 skill conformance (CS-9 platform vehicle) =="
# CS-9 relocated to platform under R-SK.9 (Architect E-20260812T190021Z).
REPO_ROOT="$(cd "${ROOT}/../../../.." && pwd)"
SA_LINT="${REPO_ROOT}/.agents/skills/harness-skill-authoring/scripts/check-skill-conformance.py"
if [ -f "${SA_LINT}" ]; then
  if ! python3 "${SA_LINT}" --flat-ok \
    "${REPO_ROOT}/.agents/skills/harness-skill-authoring"; then
    echo "FAIL: harness-skill-authoring must self-pass R-SK lint" >&2
    rc=1
  else
    echo "OK: harness-skill-authoring self-pass"
  fi
  # Tree scan includes R-SK.9 admission. Advisory unless STRICT=1
  # (CS-6 #2 / v13 category move — Architect E-20260812T104038Z).
  set +e
  python3 "${SA_LINT}" --all --flat-ok --root "${SKILLS}"
  sa_rc=$?
  set -e
  if [ "${SKILL_CONFORMANCE_STRICT:-0}" = "1" ] && [ "${sa_rc}" != "0" ]; then
    echo "FAIL: R-SK.5/R-SK.9 --all strict mode" >&2
    rc=1
  else
    echo "OK: R-SK.5/R-SK.9 --all --flat-ok (rc=${sa_rc}; STRICT=${SKILL_CONFORMANCE_STRICT:-0})"
  fi
else
  echo "FAIL: missing platform CS-9 lint at ${SA_LINT}" >&2
  rc=1
fi
