...
- Not an acceptance signal for migrated code. Real G-1…G-4 oracles need a
  provisioned specimen — use `domain-gates` / `validation-release-gates`.
...
- Final stdout line is `OK: harness-validate passed` and exit code is 0.
- **Silent-failure catch:** sections do not abort — a mid-run failure only
  surfaces as `harness-validate FAILED` on stderr plus exit 1. Never judge by
