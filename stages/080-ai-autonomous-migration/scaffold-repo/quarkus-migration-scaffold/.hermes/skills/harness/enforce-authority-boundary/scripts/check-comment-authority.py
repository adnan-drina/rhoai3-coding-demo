...
Inputs (any present):
  ...
  - .hermes/skills/harness/enforce-authority-boundary/fixtures/authority/*.jsonl     (admission fixtures)
  - --feed PATH
...
def main() -> int:
    ...
    for rel in (
        ...
        if p.is_file():
            feeds.append(p)
    fix = root / ".hermes/skills/harness/enforce-authority-boundary/fixtures/authority"
    if fix.is_dir():
