...
Exit: 0=match; 1=mismatch/unreadable (typed BLOCK); 2=missing hermes/pins.
"""
...
def main() -> int:
    ...
    try:
        ...
        print(f"FAIL: pins.json unreadable: {exc}", file=sys.stderr)
        return 2
    want = str(
    ...
    if proc.returncode != 0 and not blob:
        print("FAIL: hermes version unreadable (binary missing?)", file=sys.stderr)
        return 2
