...
M1_BODY='Follow paved-road-m1. skill_view subskills from steps.json. Attach findings-handoff.json, entry-point-inventory.json, type-inventory.json, required-extensions.json, and mta-findings.json. Happy-path terminator is kanban_request_review with reviewer set to reviewer (pass the reviewer parameter; without it the task is dispatched back to you and the paved-road audit never runs), not kanban_complete. kanban_block for external/platform (MaaS 500, missing key, GPU). Do not invent HTTP routes.'
