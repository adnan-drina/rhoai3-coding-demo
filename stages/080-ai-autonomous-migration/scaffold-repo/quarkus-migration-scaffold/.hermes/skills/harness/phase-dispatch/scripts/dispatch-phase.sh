...
#   bash .hermes/skills/harness/phase-dispatch/scripts/dispatch-phase.sh M1 --dry-run
set -euo pipefail
...
python3 "${ROOT}/.hermes/skills/harness/phase-dispatch/scripts/check-phase-input-manifest.py" "${ROOT}" "${PHASE}" \
  || die "phase input manifests failed (R0 / Operator E-20260811T113700Z)"
python3 "${ROOT}/.hermes/skills/harness/phase-dispatch/scripts/check-decision-complete-cards.py" "${ROOT}" \
...
if [[ "${PHASE}" == "M2a" ]]; then
  python3 "${ROOT}/.hermes/skills/harness/phase-dispatch/scripts/check-specify-preseed.py" "${ROOT}" \
...
4. **Partition-coverage gate** (Architect E-20260811T133858Z): run
   `python3 .hermes/skills/sdd/sdd-readiness/scripts/check-partition-coverage.py . --write-receipt migration/receipts/partition-coverage/latest.json`
...
- **Completion consumer (Operator E-20260811T120200Z):** never self-declare a binding
  ...
  `python3 .hermes/skills/harness/phase-dispatch/scripts/check-completion-na-reject.py --text "$YOUR_RESULT_SUMMARY"`.
...
4. Before `kanban_complete`: run
   `python3 .hermes/skills/harness/phase-dispatch/scripts/check-created-cards-claim.py --parent $TASK_ID --claimed <ids…>`
...
Validate: `python3 .hermes/skills/sdd/sdd-readiness/scripts/check-kanban-body.py /projects/modernized`
...
1. `bash .hermes/skills/gates/validation-release-gates/scripts/run-m4-floor.sh /projects/modernized`
2. `python3 .hermes/skills/gates/validation-release-gates/scripts/check-m4-floor-receipts.py migration/receipts/m4-floor/latest`
