...
#
# Usage:
#   bash .hermes/skills/harness/phase-dispatch/scripts/create-m3-implementer.sh \
...
python3 "${ROOT}/.hermes/skills/harness/phase-dispatch/scripts/check-create-path-tip-sync.py" "${ROOT}" \
...
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-interface-closure.py" "${ROOT}" \
  --body "${BODY_JSON}" \
  || die "INTERFACE_CLOSURE: add missing interfaces to scope/deps/dest before create"
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/stamp-destination-inventory.py" "${ROOT}" \
  --body "${BODY_JSON}" --write \
  || die "stamp-destination-inventory failed"
# Partition coverage fail-closed before create (gate already tip-loaded)
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-partition-coverage.py" "${ROOT}" \
...
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/assert-mint-constraints-complete.py" "${ROOT}" \
...
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/assert-dependency-closure.py" "${ROOT}" \
  --body "${BODY_JSON}" \
...
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-kanban-body.py" "${ROOT}" \
  --body "${BODY_JSON}" \
...
python3 "${ROOT}/.hermes/skills/harness/phase-dispatch/scripts/assert-bundle-skills-exist.py" "${ROOT}" \
...
BODY_DIGEST="$(python3 "${ROOT}/.hermes/skills/harness/auditability-repeatability/scripts/stamp-body-digest.py" \
  "${BODY_JSON}" | tail -1)"
...
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-surgical-scopes.py" "${ROOT}" "${BODY_JSON}" \
  || die "AR-4.4 surgical scopes failed for ${BODY_JSON}"
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-semantic-exits.py" "${ROOT}" "${BODY_JSON}" \
  || die "AR-2.3–2.7 semantic exits failed for ${BODY_JSON}"
# Architect E-104925Z / E-110403Z / E-111450Z — operand_count + wall-fit refuse
python3 "${ROOT}/.hermes/skills/sdd/sdd-readiness/scripts/check-operand-count.py" "${ROOT}" "${BODY_JSON}" \
  --wall-fit \
...
trap 'rm -f "${BODY_MD}"' EXIT
{
  ...
  echo "**Body immutability (Architect E-111424Z):** do **not** rewrite the typed body after dispatch. Run \`python3 .hermes/skills/harness/auditability-repeatability/scripts/check-body-digest-match.py . --body ${BODY_JSON} --expect ${BODY_DIGEST}\` — mismatch ⇒ REFUSE (\`migration/contracts/body-immutability.md\`)."
  echo "Record pre/post write-set digests under \`migration/runs/\` (schema \`rhoai3.run-journal/v1\`)."
  echo "**Checkpoint/resume (S-010 Class A #3):** before first edit, init or load \`migration/runs/<task_id>/checkpoint.json\` via \`python3 .hermes/skills/harness/auditability-repeatability/scripts/init-implementer-checkpoint.py\` / \`check-implementer-checkpoint.py\` (same dir). After each successful dest write, \`python3 .hermes/skills/harness/auditability-repeatability/scripts/stamp-implementer-checkpoint.py --completed <path>\`. On retry/re-dispatch: resume at \`next\` — do **not** cold re-walk completed operands (\`migration/contracts/implementer-checkpoint.md\`). Basename-only search is incomplete — resolve these path-anchored refs; Approval/timeout ≠ absent."
  ...
  echo "**Complete-cmd Class A (Architect E-20260811T175509Z):** before \`kanban_complete\` run \`python3 .hermes/skills/gates/validation-release-gates/scripts/assert-complete-exit-criteria.py . --task-id <this-task-id> --body ${BODY_JSON}\` — rc≠0 ⇒ REFUSE complete (\`migration/contracts/complete-cmd-exit-criteria.md\`). That script also harness-invokes body-digest + grounded-generation citation + provenance (Architect E-20260813T152211Z — do not rely on skill_view). Do not invent N/A."
  ...
  echo "**R-M3.5/7 POM handoff (Architect E-20260810T172800Z):** before first destination edit on JPA/model stories, run \`python3 .hermes/skills/sdd/sdd-readiness/scripts/check-compile-deps-preflight.py .\` — fail ⇒ typed \`dependency_wait\` (no N-file sunk cost). Scaffold/S-001-class must leave \`quarkus-hibernate-orm\` + \`quarkus-hibernate-validator\` (\`migration/contracts/pom-persistence-handoff.md\`)."
  ...
  echo "**R-M3.11 JDBC deps:** run \`python3 .hermes/skills/sdd/sdd-readiness/scripts/check-jdbc-deps-preflight.py .\` before first JDBC write — require \`spring-jdbc\` + \`spring-data-jdbc-core\` (no OOS pom)."
  ...
  echo "Progressive disclosure — do not bulk-paste skill bodies. Run: \`python3 .hermes/skills/sdd/sdd-readiness/scripts/check-kanban-body.py /projects/modernized\`"
  echo
...
python3 "${ROOT}/.hermes/skills/harness/auditability-repeatability/scripts/assert-card-body-digest-match.py" \
