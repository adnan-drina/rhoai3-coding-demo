...
trap 'rm -f "${BODY_MD}"' EXIT
{
  ...
  echo "**Complete-cmd Class A (Architect E-20260811T175509Z):** before \`kanban_complete\` run \`python3 .hermes/skills/gates/validation-release-gates/scripts/assert-complete-exit-criteria.py . --task-id <this-task-id> --body ${BODY_JSON}\` — rc≠0 ⇒ REFUSE complete (\`migration/contracts/complete-cmd-exit-criteria.md\`). Do not invent N/A."
