...
# Wave B: harness enforcement leaves moved to .hermes/enforcement/; guidance
# categories under .hermes/skills/ are the four below (no harness category).
CATEGORIES = {"analysis", "sdd", "gates", "migration"}
...
PATH_SKILL = re.compile(
    r"(?:\.hermes/)?(?:skills/(?:(analysis|sdd|gates|migration)/)?|enforcement/)"
    r"([A-Za-z0-9_-]+)(?:/|\.md|\b)"
)
...
def load_architect_keep(scaffold: Path) -> set[str]:
    ...
    candidates = [
        scaffold
        / ".hermes/enforcement/validate-contracts/references/r-sk9-architect-keep.txt",
        scaffold
