...
2. Verify body sha256 matches the card digest:
   `python3 .hermes/enforcement/record-run-evidence/scripts/check-body-digest-match.py . --body <body> --expect <digest>`
   — mismatch ⇒ REFUSE (`governance/contracts/body-immutability.md`).
...
- **DD3 story-owns-extensions:** the needing story adds its Quarkus extensions.
  Do not pre-provision hibernate/validator/flyway/jdbc on foundation for later
  stories. R-M3.5/7 persistence BOM handoff is **retired**.
