...
| Is tirith a dest control? | Operator `122315ZO` — **retired**. Managed pin `security.tirith_enabled: false` (Hermes default is `true`). KEEP `assert-no-fence-evasion`. | Cite tirith as a dest scanner; enable it; set `tirith_fail_open: false`; re-add dest-init base-PATH prepend (`112249ZA` superseded) |
