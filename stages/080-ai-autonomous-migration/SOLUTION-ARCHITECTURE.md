...
- **M5 delivery** is a separate assisted continuation: `start-m5-delivery.py` mints M5-A/B/C together (`m5:<stage>:<close_card>:<candidate16>`), native review/dispatch, finite runtime/retry. It publishes through the existing `app-push` Pipeline (no parallel deploy path). Proof is candidate → PipelineRun → image digest → ready Deployment → HTTPS Route → live checks. Deployment status is reported separately from release `ACCEPT`; outstanding qualifications from the existing release contract are not waived. Application values live in `delivery.yaml`.
