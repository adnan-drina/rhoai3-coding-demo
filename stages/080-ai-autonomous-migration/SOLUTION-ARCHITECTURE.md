...
| Slim kernel K1–K4 | Gate P-kernel **CLOSED** (Architect `142526Z`). **K1** schema+loader+validator (`d26ce74a`). **K2 REHOST** of existing `pre_tool_call.sh` (`a0616380`; MEASURED, not claimed control). **K3** mint-verifier graph procedure in `.hermes/kernel/` (this sitting; not dest live-PID; not refuse-as-control). K4 **HOLD** until Review PASS of K3. `claimed_control` stays false. Gate P-pack CLOSED |
