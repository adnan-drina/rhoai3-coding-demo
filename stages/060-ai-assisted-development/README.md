...
- **Kilo Code 7.4.8** installed as a default extension and configured for MaaS-published OpenAI-compatible endpoints. Config at `~/.config/kilo/kilo.json` (OpenCode-schema JSON) with four providers: Nemotron (default), local Qwen, qwen3-235b (16K-context external), and minimax-m2 (196K-context external). Governance rules at `~/.config/kilo/AGENTS.md`. MCP `openshift` server at `http://openshift-mcp.rhoai-mcp.svc:8080/mcp`.
