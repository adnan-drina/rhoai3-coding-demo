# Stage 060: AI-Assisted Development on the Golden Path
...
This demo stage is the first rung of the maturity ladder, and it runs entirely on platform rails: the developer enters through the developer portal service catalog, access service code base using a preconfigured IDE, extends the service with Kilo Code ("vibe coding" a new component), and exits through the CI/CD pipeline (every push to `main` runs coolstore's own `app-push` pipeline with a SonarQube gate that fails on any new issue or under-tested new code). One-shot prompting shows its power, and its limits, which motivate our next demo stage (070).
...
- **[Red Hat Developer Hub](https://developers.redhat.com/rhdh)** provides the developer portal, catalog, and golden-path templates.
