...
This demo stage is the first rung of the maturity ladder, and it runs entirely on platform rails: the developer enters through the portal catalog, extends the service with Kilo Code ("vibe coding" a new component), and exits through the pipeline (every push to `main` runs coolstore's own `app-push` pipeline with a SonarQube gate that fails on any new issue or under-tested new code). One-shot prompting shows its power — and its limits, which motivate Stage 070.
