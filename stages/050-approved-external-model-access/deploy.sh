...
ensure_namespace "redhat-ods-applications"

if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    ensure_secret_from_env "openai-api-key" "redhat-ods-applications" "api-key=${OPENAI_API_KEY}"
    oc label secret openai-api-key -n redhat-ods-applications inference.networking.k8s.io/bbr-managed=true --overwrite
    log_success "openai-api-key provisioned in redhat-ods-applications namespace"
else
...
echo "  oc get externalmodel,maasmodelref -n redhat-ods-applications"
echo ""
