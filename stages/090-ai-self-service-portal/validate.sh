...
check_argocd_app "090-ai-self-service-portal"
...
APP_REPO_URL=$(oc get application 090-ai-self-service-portal -n openshift-gitops -o jsonpath='{.spec.source.repoURL}' 2>/dev/null || echo "")
APP_TARGET_REVISION=$(oc get application 090-ai-self-service-portal -n openshift-gitops -o jsonpath='{.spec.source.targetRevision}' 2>/dev/null || echo "")
REPO_NO_GIT="${APP_REPO_URL%.git}"
EXPECTED_CATALOG_URL=""
if [[ "$REPO_NO_GIT" =~ ^https://github.com/([^/]+)/([^/]+)$ ]] && [[ -n "$APP_TARGET_REVISION" ]]; then
    EXPECTED_CATALOG_URL="https://raw.githubusercontent.com/${BASH_REMATCH[1]}/${BASH_REMATCH[2]}/${APP_TARGET_REVISION}/gitops/stages/090-ai-self-service-portal/base/catalog/all.yaml"
fi
