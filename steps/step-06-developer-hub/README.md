...
Users see **"Sign in using OIDC"** on the RHDH login page. After clicking, they authenticate with their OpenShift credentials (same `ai-admin` / `ai-developer` / `redhat123` used across MTA and Dev Spaces).
...
3. Authenticate with OpenShift credentials (`ai-admin` / `redhat123`)
